...
#include "/usr/include/c++/7/bits/unique_ptr.h"
...
protected:
	...
	void labeling(const cv::Mat &points_, // The input data points
		...
		const std::vector<std::vector<cv::DMatch>> &neighbors_, //do_local_optimization The neighborhood
	...
	Model &obtained_model_)  // The output model 
{
	...
	*/
	int exist_best_model = 0;
	int best_mode_valid = 0;
		...
		statistics.iteration_number < MIN(max_iteration, settings.max_iteration_number))
	{
		...
		//cout<<statistics.iteration_number<<"     models size: "<<models.size()<<endl;
		for (const auto &model : models)
		{
			...
			if (isScoreLess(so_far_the_best_score, // The so-far-the-best model's score
				score)) exist_best_model = 1;
			
			// Store the model of its score is higher than that of the previous best
			//问题在于，so_far_the_best_model存在，但其无效！！！！ 导致最后一次优化无法做
			if (isScoreLess(so_far_the_best_score, // The so-far-the-best model's score
			...
			do_local_optimization) // A flag to decide if all the criteria meet to apply local optimization
		{
			...
			//cout<<"so_far_the_best_model.descriptor 1:"<<endl;
	...
	// Apply a final local optimization if it hasn't been applied ye
	
	if(exist_best_model == 1 && best_mode_valid == 0){
	...
	if (settings.do_local_optimization && statistics.local_optimization_number == 0 )
	{
	  ...
	  /*
	     if(so_far_the_best_model.inliers.size() == 0){
		  cout<<"bug->2"<<endl;
		 // return;
	     }
	  */
		...
		//if(so_far_the_best_model.inliers.size() == 0)
		//  cout<<"2:empty"<<endl;
		// cout<<"so_far_the_best_model.descriptor 2:"<<endl;
		// Graph-cut-based local optimization 
		graphCutLocalOptimization(points_, // All points
			temp_inner_inliers[inl_offset], // Inlier set of the current so-far-the-best model
			so_far_the_best_model, // Best model parameters
	...
	//cout<<"iteration_number: "<<statistics.iteration_number<<endl;
}
...
void GCRANSAC<_ModelEstimator, _NeighborhoodGraph>::labeling(const cv::Mat &points_, 
	...
	double &energy_)
{
	//cout<<"gc-threshold_:"<<threshold_<<endl;
